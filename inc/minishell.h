...
int			add_firsts_if_not_token(t_parser *data, t_rule *next_rule,
...
int			should_look_for_next_right_symbol(t_symbol *left_symbol,
				t_rule *rule);
void		get_first_dfs(t_parser *data, t_rule *rule, t_symbol *left_symbol,
...
t_symbol	*get_symbol_from_list(t_parser *data, t_list *right_symbols,
				int dot_pos);
...
void	fill_table_when_reduce(t_parser *data, int ***table, t_list *state,
			t_list *item);
